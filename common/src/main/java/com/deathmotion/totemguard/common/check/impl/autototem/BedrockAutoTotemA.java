...
@CheckData(description = "Impossible Bedrock click time difference", type = CheckType.AUTO_TOTEM, experimental = true)
public class BedrockAutoTotemA extends CheckImpl implements EventCheck {

    private Long popTimestamp;
    private Long pickupTimestamp;

    public BedrockAutoTotemA(TGPlayer player) {
        super(player);
    }

    private boolean shouldEvaluate() {
        return player.isBedrockPlayer() && platform.getConfigRepository().bedrock().isEnabled();
    }
    ...
    private void evaluate(long placedAt) {
        ...
        int maxClickDiff = bedrock.getInt(getName(), "max-click-diff-ms", 15);
        int maxUseDiff = bedrock.getInt(getName(), "max-use-to-place-diff-ms", 1500);

        boolean flagged = clickDiff >= 0 && clickDiff <= maxClickDiff && useDiff >= 0 && useDiff <= maxUseDiff;
        BedrockDebugLogger.logClickTiming(player, clickDiff, useDiff, flagged);
